...
///
/// </summary>
internal class FindMinimumInRotatedSortedArrayIISolution
{
    public void Test()
    {
...
public class Solution {
    public int FindMin(int[] nums) {
            if (nums.Length == 0)
                return 0;
            return MinArray_Help(nums, 0, nums.Length - 1);
    }

        public int MinArray_Help(int[]numbers, int l, int r)
        {
            int mid = l + (r - l) / 2;
            ...
            if (numbers[l] >= numbers[mid])
                res = Math.Min(res, MinArray_Help(numbers, l, mid));
            else
                res = Math.Min(res, numbers[l]);

            if (numbers[mid] >= numbers[r])
                res = Math.Min(res, MinArray_Help(numbers, mid, r));
            else
