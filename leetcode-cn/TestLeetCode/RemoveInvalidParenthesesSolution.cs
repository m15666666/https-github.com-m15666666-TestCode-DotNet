...
///
/// </summary>
internal class RemoveInvalidParenthesesSolution
{
    public void Test()
    {
    ...
    public IList<string> RemoveInvalidParentheses(string s)
    {
        ...
        void Dfs(int index, int leftCount, int rightCount, int leftRem, int rightRem, StringBuilder builder)
        {
            if (index == len)
            {
            ...
            if (character != '(' && character != ')') Dfs(index + 1, leftCount, rightCount, leftRem, rightRem, builder);
            else if (character == '(') Dfs(index + 1, leftCount + 1, rightCount, leftRem, rightRem, builder);
            else if (rightCount < leftCount) Dfs(index + 1, leftCount, rightCount + 1, leftRem, rightRem, builder);
...
时间复杂度：O(2^N)O(2 
N
 )。因为在最坏的情况下，表达式中只有左括号，对于每个括号，我们都有两个选项，即是删除还是考虑它。考虑到表达式有 NN 括号，时间复杂性将为O(2^N)O(2 
N
 )。
空间复杂度：O(N)O(N)，因为我们使用的是递归解决方案，对于递归解决方案，在递归过程中，始终有堆栈空间用作内部函数状态保存到堆栈中。递归的最大深度决定了所使用的堆栈空间。因为我们一次处理一个字符，而递归的基本情况是当我们处理完表达式字符串的所有字符时，堆栈的大小将为 O(n)O(n)。注意，我们不考虑存储有效表达式所需的空间。我们只计算中间过程的空间。
方法二：有限的回溯！
...
表达式在基本情况下有效的条件现在将变为left_rem == 0 and right_rem == 0。注意，我们不必再检查 left_count == right_count 了，因为在有效表达式的情况下，在递归结束时，我们会删除所有放错位置或无效的括号。所以，只有当 left_rem == 0 and right_rem == 0 时，我们才需要检查。
...
时间复杂度：我们所执行的优化只是一种更好的修剪形式。在最坏的情况下，我们可以有 ((((((((( 和 left_rem = len(S) 和在这种情况下我们可以丢弃所有字符，因为所有字符都放错了位置。在最坏的情况下，每个括号中仍然有两个选项，这给了我们 O(2^n)O(2 
n
 ) 的复杂性。
空间复杂度：空间复杂性与以前的解决方案保持相同，即 O(N)O(N)。 在到达基本情况之前，我们必须达到 NN 的最大递归深度。注意，我们不考虑存储有效表达式所需的空间。我们只计算中间过程空间。
