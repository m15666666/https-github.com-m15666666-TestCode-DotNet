...
时间复杂度：O(n^2)O(n 
2
 )，其中 nn 是给定的正整数。对于从 22 到 nn 的每一个整数都要计算对应的 \text{dp}dp 值，计算一个整数对应的 \text{dp}dp 值需要 O(n)O(n) 的时间复杂度，因此总时间复杂度是 O(n^2)O(n 
2
 )。

空间复杂度：O(n)O(n)，其中 nn 是给定的正整数。创建一个数组 \text{dp}dp，其长度为 n+1n+1。
...
使用上述状态转移方程，计算 \text{dp}[i]dp[i] 时，jj 的值遍历了从 11 到 i-1i−1 的所有值，因此总时间复杂度是 O(n^2)O(n 
2
 )。是否可以降低时间复杂度？

上述状态转移方程包含两项，当 jj 固定时，\text{dp}[i]dp[i] 的值由 j \times (i-j)j×(i−j) 和 j \times \text{dp}[i-j]j×dp[i−j] 中的较大值决定，因此需要对两项分别考虑。

首先考虑 j \times \text{dp}[i-j]j×dp[i−j] 这一项。

注意到 \text{dp}dp 的定义，\text{dp}[i]dp[i] 表示将正整数 ii 拆分成至少两个正整数的和之后，这些正整数的最大乘积，因此对于任意 1 \le j < i1≤j<i，有 \text{dp}[i] \ge j \times \text{dp}[i-j]dp[i]≥j×dp[i−j]。
...
j+1
​	
 ，因此 \text{dp}[i] \geq \frac{j-1}{2} \times \text{dp}[i - \frac{j-1}{2}] \ge \frac{j-1}{2} \times \frac{j+1}{2} \times \text{dp}[i-j]dp[i]≥ 
2
j−1
​	
 ×dp[i− 
...
j
​	
 ，因此 \text{dp}[i] \ge \frac{j}{2} \times \text{dp}[i - \frac{j}{2}] \ge \frac{j}{2} \times \frac{j}{2} \times \text{dp}[i-j]dp[i]≥ 
2
j
​	
 ×dp[i− 
...
由此可知，如果 j \ge 4j≥4，则 \text{dp}[j] \ge jdp[j]≥j，当且仅当 j=4j=4 时等号成立，即当 j \ge 4j≥4 时一定能将 jj 拆成至少两个正整数的和，这些正整数的乘积大于或等于 jj。

同时也可以得到，如果 j \ge 4j≥4，则 \text{dp}[i] \ge j \times \text{dp}[i-j]dp[i]≥j×dp[i−j]，只有当 j=4j=4 时等号可能成立。又由于

\text{dp}[i] \ge 2 \times \text{dp}[i-2] \ge 2 \times (2 \times \text{dp}[i-4]) = 4 \times \text{dp}[i-4]
dp[i]≥2×dp[i−2]≥2×(2×dp[i−4])=4×dp[i−4]

因此取 j=2j=2 计算得到的 \text{dp}[i]dp[i] 一定不会小于取 j=4j=4 计算得到的 \text{dp}[i]dp[i]。根据上述分析，j \ge 4j≥4 的情况都是不需要考虑的。

那么 j=1j=1 是否需要考虑？答案是不需要。如果取 j=1j=1，则有 \text{dp}[i] \ge 1 \times \text{dp}[i-1]=\text{dp}[i-1]dp[i]≥1×dp[i−1]=dp[i−1]。当 i \ge 3i≥3 时，\text{dp}[i-1]dp[i−1] 是将正整数 i-1i−1 拆分成至少两个正整数的和之后，这些正整数的最大乘积，在拆分成的正整数中，任选一个数字加 11，则拆分成的正整数的和变成 ii，且乘积一定大于 \text{dp}[i-1]dp[i−1]，因此必有 \text{dp}[i]>\text{dp}[i-1]dp[i]>dp[i−1]，即当 j=1j=1 时不可能得到最大的 \text{dp}[i]dp[i] 的值。

根据上述分析可知，计算 \text{dp}[i]dp[i] 的值只需要考虑 j=2j=2 和 j=3j=3 的情况，不需要遍历从 11 到 i-1i−1 的所有值。

其次考虑 j \times (i-j)j×(i−j) 这一项。

根据上述推导可知，如果 j \ge 4j≥4，则 \text{dp}[j] \ge jdp[j]≥j，当且仅当 j=4j=4 时等号成立。因此在 i-j \ge 4i−j≥4 的情况下，有 \text{dp}[i-j] \ge i-jdp[i−j]≥i−j，\text{dp}[i] \ge j \times \text{dp}[i-j] \ge j \times (i-j)dp[i]≥j×dp[i−j]≥j×(i−j)，此时计算 \text{dp}[i]dp[i] 的值不需要考虑 j \times (i-j)j×(i−j) 的值。

如果 i-j<4i−j<4，在计算 \text{dp}[i]dp[i] 的值的时候就需要考虑 j \times (i-j)j×(i−j) 的值。在考虑 j \times \text{dp}[i-j]j×dp[i−j] 时，根据上述分析，只需要考虑 j=2j=2 和 j=3j=3 的情况。在考虑 j \times (i-j)j×(i−j) 时，需要考虑 jj 的哪些值？

如果 j=1j=1，则 1 \times (i-1)=i-11×(i−1)=i−1，当 i=2i=2 或 i=3i=3 时有 \text{dp}[i]=i-1dp[i]=i−1，当 i \ge 4i≥4 时有 \text{dp}[i] \ge i>i-1dp[i]≥i>i−1，显然当 i \ge 4i≥4 时取 j=1j=1 不可能得到最大乘积，因此 j=1j=1 时是不需要考虑的。

如果 j \ge 4j≥4，\text{dp}[i]dp[i] 是否可能等于 j \times (i-j)j×(i−j)？当 ii 固定时，要使得 j \times (i-j)j×(i−j) 的值最大，jj 的值应该取 j=i/2j=i/2，这里的 // 表示整数除法。当 j \ge 4j≥4 时，若要满足 j=i/2j=i/2，则 i \ge 8i≥8，此时 i-j \ge 4i−j≥4，利用上述结论，\text{dp}[i-j] \ge i-jdp[i−j]≥i−j，因此 j \times \text{dp}[i-j] \ge j \times (i-j)j×dp[i−j]≥j×(i−j)。由此可见，当 j \ge 4j≥4 时，计算 \text{dp}[i]dp[i] 只需要考虑 j \times \text{dp}[i-j]j×dp[i−j]，不需要考虑 j \times (i-j)j×(i−j)。

又由于在使用 j \times \text{dp}[i-j]j×dp[i−j] 计算 \text{dp}[i]dp[i] 时，j=2j=2 和 j=3j=3 的情况一定优于 j \ge 4j≥4 的情况，因此无论是考虑 j \times \text{dp}[i-j]j×dp[i−j] 还是考虑 j \times (i-j)j×(i−j)，都只需要考虑 j=2j=2 和 j=3j=3 的情况。
...
时间复杂度：O(n)O(n)，其中 nn 是给定的正整数。和方法一相比，计算每个整数对应的 \text{dp}dp 的值的时间复杂度降到 O(1)O(1)，因此总时间复杂度降到 O(n)O(n)。
...
时间复杂度：O(1)O(1)。涉及到的操作包括计算商和余数，以及幂次运算，时间复杂度都是常数。

空间复杂度：O(1)O(1)。只需要使用常数复杂度的额外空间。
...
public class Solution {
    public int IntegerBreak(int n) {
        ...
        if (mod == 0) last = 3;
            ...
            last = (1 + mod) * 2;
        int result = (int)Math.Pow(3, count - 1) * last;
        return result;
    }
}
