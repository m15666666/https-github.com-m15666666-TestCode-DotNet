...
///
/// </summary>
internal class BasicCalculatorSolution
{
    public void Test()
    {
    }
    ...
    public int Calculate(string s)
    {
        ...
        return Calculate(s.GetEnumerator());

        int Calculate(CharEnumerator text)
        {
            int operand = 0;
            int result = 0;
            int sign = 1;
            ...
            while (text.MoveNext())
            {
                ...
                if (c == '(')
                {
                    var r = Calculate(text);
                    result += sign * r;
                    continue;
                }

                if (c == ')') return result + sign * operand;
            }
