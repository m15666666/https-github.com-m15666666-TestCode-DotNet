...
///
/// </summary>
internal class BestTimeToBuyAndSellStockIIISolution
{
    public void Test()
    {
    ...
    public int MaxProfit(int[] prices)
    {
        ...
        int[,] firstDay = new int[2, 3]; // 持股或不持股，卖出0，1，2次
        int[,] secondDay = new int[2, 3]; // 持股或不持股，卖出0，1，2次

        //firstDay[0, 0] = 0;
        firstDay[1, 0] = -prices[0];
        firstDay[0, 1] = int.MinValue;
        firstDay[0, 2] = int.MinValue;
        firstDay[1, 1] = int.MinValue;
        //firstDay[1, 2] = int.MinValue;

        for( int i = 1; i < len; i++)
        {
            var p = prices[i];
            //secondDay[0, 0] = 0;
            secondDay[0, 1] = Max(firstDay[1,0], p, firstDay[0,1]);
            secondDay[0, 2] = Max(firstDay[1,1], p, firstDay[0,2]);
            secondDay[1, 0] = Math.Max(/*firstDay[0,0]*/ -p, firstDay[1,0]);
            secondDay[1, 1] = Max(firstDay[0,1], -p, firstDay[1,1]);
            //secondDay[1, 2] = int.MinValue;

            firstDay = secondDay;
        }

        return Math.Max(Math.Max(firstDay[0,1],firstDay[0,2]), 0);
        int Max( int p1, int plus, int p2)
        {
            if (p2 == int.MinValue && p1 == int.MinValue) return int.MinValue;
            return Math.Max(p1 + plus, p2);
        }
    }
...
public class Solution {
    public int MaxProfit(int[] prices) {
        ...
        // 根据提议： 寻找出ikj三个变量的组合，并取其最大值即可
        // 转移：
            // dp[i][k][0]= dp[i-1][k][0], dp[i-1][k][1]+price
            // dp[i][k][1]= dp[i-1][k][1], dp[i-1][k-1][0]+price  // buy behaviour
        ...
        //base case - Day0
        var dp_10=0;
        var dp_11=int.MinValue;
        var dp_20=0;
        var dp_21=int.MinValue;
    ...
    public int MaxProfit(int[] prices) {
        ...
        for(int i=1;i<prices.Length;i++)
        {
            if(minprices>prices[i])
                minprices=prices[i];
            leftmaxprofit[i]=Math.Max(prices[i]-minprices,leftmaxprofit[i-1]);
        }
        ...
        for(int j=prices.Length-2;j>0;j--)
        {
            if(maxprices<prices[j])
                maxprices=prices[j];
            rightprofit[j]=Math.Max(maxprices-prices[j],rightprofit[j+1]);
        }
        int maxprofit=0;
        for(int i=0;i<prices.Length;i++)
        {
            maxprofit=Math.Max(maxprofit,leftmaxprofit[i]+rightprofit[i]);
        }
        return maxprofit;
    }
}
...
public class Solution {
        ...
        {
            var dp_i_2_0 = 0;
            var dp_i_2_1 = int.MinValue;
            var dp_i_1_0 = 0;
            var dp_i_1_1 = int.MinValue;
