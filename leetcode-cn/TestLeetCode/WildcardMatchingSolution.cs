...
class WildcardMatchingSolution
{
    ...
    public bool IsMatch(string s, string p) 
    {
        ...
        if (string.IsNullOrEmpty(p) || string.IsNullOrEmpty(s)) return false;
        ...
        int sIdx = 0, pIdx = 0;
        int pStackedIdx = -1, sStackedIdx = -1;
        while (sIdx < sLen) {
            if (pIdx < pLen)
            {
                var pc = p[pIdx];
...
记忆。这是优化递归的标准方法。我们使用 (s, p) 作为键，使用匹配与不匹配作为布尔值。创建一个记忆的哈希映射。将所有已经检查的 (s, p) 保留在哈希映射中。这样，如果有任何重复的检查，只需查看哈希表，而不需再次进行计算。
清理输入数据，不管 a****bc**cc 中有多少个星号，它们都可以简化为 a*bc*cc。这样的清理有助于减少递归深度。
算法：
...
min(S,P/2)
 )。其中 SS 和 PP 指的是输入字符串和字符模式的长度。 最好的情况很明显，让我们估算最坏的情况。最耗时的递归是字符模式上的星号形成树的情况，将执行两个分支 helper(s, p[1:]) 和 helper(s[1:], p)。数据清理后字符模式中的最大星树为 P/2P/2，因此时间复杂度为 \mathcal{O}(2^{\min(S, P/2)})O(2 
min(S,P/2)
 )。
空间复杂度：\mathcal{O}(2^{\min(S, P/2)})O(2 
min(S,P/2)
 )，用来存储记忆哈希表和递归调用堆栈。
方法二：动态规划
...
空间复杂度：\mathcal{O}(S P)O(SP)，用来存储匹配表格。
方法三：回溯
复杂度 \mathcal{O}(S P)O(SP) 比 \mathcal{O}(2^{\min(S, P/2)})O(2 
min(S,P/2)
 ) 好的多，但是仍然有改进的余地。不需要计算整个表格，也就是检查每个星号的所有可能性：
...
时间复杂度：最好的情况下是 \mathcal{O}(\min(S, P))O(min(S,P))，平均情况下是 \mathcal{O}(S \log P)O(SlogP)，其中 SS 和 PP 指的是字符模式和输入字符串的长度。详细证明可点击：证明过程。
