...
///
/// </summary>
internal class ExpressionAddOperatorsSolution
{
    public void Test()
    {
    }
    ...
    public IList<string> AddOperators(string num, int target)
    {
        ...
        Dfs(0, 0, 0, 0, new List<string>());

        return answer;

        void Dfs(int index, long previousOperand, long currentOperand, long value, List<string> ops)
        {
            var nums = num;
            ...
            if (index == len)
            {
                ...
                {
                    builder.Length = 0;
                    for (int i = 1; i < ops.Count; i++) builder.Append(ops[i]);
            ...
            if (0 < currentOperand) Dfs(index + 1, previousOperand, currentOperand, value, ops);
            ...
            if (0 < ops.Count)
            {
                ...
                ops.Add("*");
                ops.Add(current_val_rep);
                long multiple = currentOperand * previousOperand;
                Dfs(index + 1, multiple, 0, value - previousOperand + multiple, ops);
                ops.RemoveAt(ops.Count - 1);
                ops.RemoveAt(ops.Count - 1);
            }
        }
...
我们合并这些分区的方法是考虑第四个运算符，它只向前移动一步，将当前操作数扩展一位。实际上，在我们的实现中，从 12 --> 12312−−>123 开始是一个无操作操作数。(12*10)+3(12∗10)+3。
...
如上所述，我们有多种选择，可以选择使用什么运算符，可以选择哪些操作数，因此，我们必须考虑找到所有有效表达式的所有可能性。
我们的递归调用 index ，它表示我们在原始 nums 字符串中看到的当前数字，以及到目前为止构建的表达式字符串。
在每一步中，我们都有 4 个不同的递归调用。NO OP 调用只是将当前的操作数扩展到当前数字并向前移动。其余的递归调用对应于+、-、和 *。
我们继续这样构建表达式，最终会处理整个 nums 字符串。那时，我们检查到现在为止构建的表达式是否是有效的表达式，如果是有效的表达式，我们会记录它。
...
是的。这就是我们要遵循的理念。除了跟踪表达式字符串是什么，我们还将跟踪它的值，以便当递归到达基本情况时，我们可以通过 O(1)O(1) 的时间复杂度来判断表达式是否等于目标值
...
时间复杂度：
在这一过程的每一步中，我们都会考虑 4 种不同的选择或 4 种不同的递归路径。基本情况是当索引值达到 NN 时，即 nums 数组的长度。因此，我们的复杂性将是O(4^n)O(4 
n
 )。
对于基本情况，我们在 Java 中使用了 StringBuilder::toString 函数和在 Python 中使用了 .join() 函数花费了 O(N)O(N) 时间。这里 NN 表示了表达式的长度。在最坏的情况下，每个数字都是一个操作数，我们有 NN 个数字和 N-1N−1 个运算符。所以 O(N)O(N)是对于一个表达式。在最坏的情况下，我们可以有 O(4^n)O(4 
n
 ) 有效表达式。
空间复杂度：
...
public class Solution {
    ...
    //pos:字符串开始的位置
    //eval：目前为止计算的结果
    private void BackTrace(string path, string num, int target, int pos,
		...
		for(int i=pos; i<num.Length; i++){
			if(i>pos && num[pos]=='0') // 数字不能以 0 开头
				break;
			var str=num.Substring(pos, i+1-pos);
			long cur=long.Parse(str);
			if(pos==0)
				BackTrace(path+str, num, target, i+1, cur, cur);
			else{
				BackTrace(path+"+"+str, num, target, i+1, eval+cur, cur);
				BackTrace(path+"-"+str, num, target, i+1, eval-cur, -cur);
				BackTrace(path+"*"+str, num, target, i+1, eval-multed+multed*cur,
					multed*cur);
			}
		}
	}
