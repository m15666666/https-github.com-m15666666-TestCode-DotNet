...
/// 
/// </summary>
class NextGreaterNodeInLinkedListSolution
{
    public void Test()
    {
    ...
    public int[] NextLargerNodes(ListNode head)
    {
        ...
        int[] biggerValues = new int[MaxCount];
        ...
        while (head != null)
        {
            value = head.val;
            values[index] = value;
            while (0 < indexs.Count && values[indexs.Peek()] < value)
            {
                biggerValues[indexs.Pop()] = value;
            }
            indexs.Push(index);
            index++;
            head = head.next;
        ...
        while (0 < indexs.Count) biggerValues[indexs.Pop()] = 0;

        int[] ret = new int[index];
        for (int i = 0; i < index; i++) ret[i] = biggerValues[i];
        return ret;
    }
}