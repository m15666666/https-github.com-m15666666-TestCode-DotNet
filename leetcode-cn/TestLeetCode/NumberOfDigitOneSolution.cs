...
///
/// </summary>
internal class NumberOfDigitOneSolution
{
    public int CountDigitOne(int n)
    {
        long ret = 0;
        for (long i = 1, divider = 10; i <= n; i *= 10, divider *= 10)
            ret += (n / divider) * i + Math.Min(Math.Max(n % divider - i + 1, 0), i);
        return (int)ret;
    }
}
...
public class Solution {
    public int CountDigitOne(int n) {
        ...
        while (i <= n) {
            long j = i * 10;
            ans += (int)(n / j * i + Math.Min(Math.Max(n % j - i + 1, 0), i));
            i = j;
        }
        return ans;
    }
