...
///
/// </summary>
internal class RepeatedSubstringPatternSolution
{
    public void Test()
    {
    ...
    public bool RepeatedSubstringPattern(string s)
    {
        ...
        static bool Kmp(string pattern)
        {
            ...
            for (int i = 1; i < n; ++i)
            {
                char c = pattern[i];
                int j = fail[i - 1];
                while (j != StartPoint && pattern[j + 1] != c)
                    j = fail[j];

                fail[i] = pattern[j + 1] == c ? j + 1 : StartPoint;
            }

            int lastFail = fail[n - 1];
            if (lastFail == StartPoint) return false;
            int cycleCount = n - (lastFail + 1);
            return n % cycleCount == 0;
