namespace TestLeetCode.BasicDataStructure
{
    ...
    /// 参考：https://www.cnblogs.com/abatei/archive/2008/11/17/1335031.html
    /// </summary>
    public class AVLTree1
    {
        ...
        {   //如果是空树，则新结点成为二叉排序树的根
            if (Root == null)
            {
                Root = new Node { Value = value,BF = 0 };
                return true;
            }
            p = 0;
            ...
            while (current != null)
            {
                ...
                current = (value < prev.Value) ? prev.Left : prev.Right;
            }
            current = new Node { Value = value, BF = 0 }; //创建新结点
            if (value < prev.Value) prev.Left = current; //成为左孩子
            else prev.Right = current; //成为右孩子
        ...
        private void RemoveNode(Node node)
        {
            ...
            {   //bf表示平衡因子的改变量，当删除的是左子树中的结点时，平衡因子-1
                //当删除的是右子树的孩子时，平衡因子+1
                int bf = (data <= path[p - 1].Value) ? -1 : 1;
                path[--p].BF += bf; //改变当父结点的平衡因子
                bf = path[p].BF; //获取当前结点的平衡因子
        ...
        private bool RotateSubTree(int bf)
        {
            ...
            {
                Root = newRoot; //如果旋转根为AVL树的根，则指定新AVL树根结点
            }
            return tallChange;
        }
